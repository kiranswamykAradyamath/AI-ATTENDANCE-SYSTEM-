...
from src.database.db import DatabaseConnectionError, get_subject_students
...
def voice_attendance_dialog(selected_subject_id):
    ...
    if st.button("Analyze Audio", width="stretch", type="primary"):
        ...
        # Lazy import — prevents module-level crash if torch/resemblyzer unavailable
        try:
            from src.pipelines.voice_pipelines import process_bulk_audio
        except Exception as exc:
            st.error(
                f"⚠️ Voice recognition is not available: {exc}\n\n"
                "Please ensure torch, torchaudio, and resemblyzer are installed."
            )
            return
