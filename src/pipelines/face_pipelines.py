...
def load_face_model():
    import os
    # Fix Windows UnicodeEncodeError caused by DeepFace emoji log messages
    ...
    try:
        ...
        st.error(
            f"⚠️ Could not load face recognition model: {exc}\n\n"
            "If this is a first-time setup, the model weights may still be downloading. "
...
def get_face_embeddings(image_np):
    ...
    if model is None:
        # Model failed to load — error already shown by load_face_model()
        return []
